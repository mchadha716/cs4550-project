...
## Experiment 2: HTML Geolocation API
	For this experiment, we tested the ability to obtain the user's current location from JavaScript. Source code for this experiment can be found in the web-ui folder of the mbta_tracker. We have put the code in place to be able to send and receive data over sockets. 

	From this experimet, The device's current location is displayed after the user consents. There is a delay in obtaining the initial position of the device. We have also learned, since the UI is operating separately from the server, we do not know how to communicate over sockets and channels. We are hoping this will be covered in the next lesson as we finish going over Single-Page Applications. 
